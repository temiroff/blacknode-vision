...
Transport adapters for the `imu` component of `blacknode-perception`.
...
Declare it in `blacknode-package.toml`:
    ...
    [components.imu.adapters.ros2]
    description = "ROS 2 adapter for imu."
    default = false
    capabilities = ["adapter.imu.ros2"]
    nodes = ["components/imu/adapters/ros2/nodes"]
